...
This reimplements the KNX cloud flow that ETS runs through the native ``Knx.CloudKit`` module,
so a project certificate (``{pid}.certificate``) can be requested from macOS/Linux without ETS or
a dongle -- using the user's *own* MyKnx account (legitimate use of the user's license).

Status of each part (reverse-engineered from ETS 6.3 assemblies + the decrypted native DLL
``Knx.CloudKit64.decrypted.dll``; see ``.references/ETS-SIGNING-AND-LICENSING.md``):

VERIFIED (from strings / managed decompile):
  * OAuth2 device-code flow, auth server ``https://id.knx.org`` (RFC 8628):
    ``client_id = "ets-63"``, ``client_secret = "ot9sa#thiegeiyeeleirei5-pheiWo6ci"``,
    routes ``oauth/devicecode`` and ``oauth/accesstoken`` (port 443).
  * API server ``https://openapi.knx.org/v1``. Session start returns ``{"session_id": ...}``;
    subsequent requests carry ``x-session-id`` and a rotating one-time token header
    ``x-next-ot-token`` (the response provides the next token). ``accept: application/json``.
  * Certificate request is workset-scoped (see SOLVED below); response contains the signed
    ``{pid}.certificate`` (server-side signing -- there is no local key).
  * License key/product listing: ``GET .../product/getAll`` and ``.../product/{productId}/getKey``.

LIVE-TESTED (2026-08-31, real MyKnx account + mitmproxy capture of ETS 6.4.1):

  * **Session + auth handshake SOLVED** (all plain JSON over TLS -- NO app-layer encryption):
    - ``POST /v1/user/login?username=<u>&password=<p>`` -> 200 "Login successful", response headers
      ``X-Session-ID`` + ``X-Next-OT-Token``.
    - Every authed request then sends ``X-Session-ID`` + ``X-OT-Token`` (the CURRENT one-time
      token); the response returns ``X-Next-OT-Token``, which becomes the next request's
      ``X-OT-Token``. (My earlier 401 was sending ``x-next-ot-token`` as the request header -- wrong
      name. Fixed in ``_headers``/``_absorb``.)
    - Verified 200: ``GET /v1/ping`` -> "pong", ``GET /v1/entity/getAll``,
      ``GET /v1/product/getAll`` / ``product/getForEntity?entityId=``, ``PATCH /v1/session/{id}``,
      ``GET /v1/workset/getForEntity?entityId=``, ``GET /v1/user/logout``.
  * ETS itself logs in via the OAuth device-code flow (id.knx.org) and exchanges it for the same
    session; username/password login is an equivalent path to a session and is what this client uses.

SOLVED -- certificate endpoint (from Ghidra decompile of the native cert function in
``Knx.CloudKit64.decrypted.dll`` + live route probing):
  * The cert path is **workset-scoped**. Full flow (all routes return 2xx live):
    ``POST /v1/workset`` -> ``POST /v1/workset/{ws}/claim`` -> ``POST /v1/workset/{ws}/product/{productId}``
    -> ``POST /v1/workset/{ws}/product/{productId}/certificate`` with body
    ``{"projectHash": "<hash>", "projectName": "<name>"}`` -> ``POST /v1/workset/{ws}/release``.
    The native code builds the URL as ``/workset/`` + wsId + ``/product/`` + prodId + ``/certificate``.
  * REQUIRES a cloud-enabled ETS product license. With a non-ETS product the add-product step
    returns HTTP 422 ("Encryption \"cloud\" is not enabled for product_type ..."), so the actual
    cert signing could not be captured on the test account -- but every route in the chain is
    confirmed and the request/response shape is fixed by the decompile.

STILL UNVERIFIED:
  * The exact ``projectHash`` input (here: SHA-256 of the ``{pid}.signature`` bytes) -- could not be
    pinned because the test account cannot mint a cert (422 above).
"""
...
AUTH_BASE = "https://id.knx.org"
API_BASE = "https://openapi.knx.org/v1"
CLIENT_ID = "ets-63"
CLIENT_SECRET = "ot9sa#thiegeiyeeleirei5-pheiWo6ci"
...
def project_hash(folder_signature: bytes) -> str:
    ...
    ``project.xml`` + ``0.xml``). The certificate binds to that signature (see ai6/ak0 in the
    decompile), so hashing it is the most likely input -- confirm against a real request. A leading
...
class MyKnxSession:
    ...
    def oauth_login(self, jwt_access_token: str) -> None:
        """Establish a session from an OAuth device-code JWT (verified live: what ETS 6.4.1 does).

        ``POST /v1/user/oauthLogin?token=<jwt>`` -> 200 + ``X-Session-ID`` + ``X-Next-OT-Token``.
        Use this when the token came from the id.knx.org device-code flow (:func:`device_code_login`).
        """
        import urllib.parse

        qs = urllib.parse.urlencode({"token": jwt_access_token})
        status, headers, body = _post(
            f"{API_BASE}/user/oauthLogin?{qs}",
            b"",
            self._headers(json_body=False),
            self.timeout,
        )
        if status // 100 != 2:
            raise RuntimeError(f"oauthLogin failed: HTTP {status}: {body[:200]!r}")
        self._absorb(headers)
        if not self.session_id:
            raise RuntimeError("oauthLogin returned no x-session-id")
...
def device_code_login(
    ...
    """Run the OAuth2 device-code flow; return an access token.

    ``on_prompt(verification_uri, user_code)`` is called once with the URL and code for the user to
    authorise in a browser with their own MyKnx account (defaults to ``print``); then this polls the
    token endpoint until authorised.
    """
    body = f"client_id={CLIENT_ID}&client_secret={CLIENT_SECRET}".encode()
    status, _, resp = _post(
        f"{AUTH_BASE}/oauth/devicecode",
        body,
        {
    ...
    if status // 100 != 2:
        raise RuntimeError(f"devicecode request failed: HTTP {status}: {resp[:200]!r}")
    dc = json.loads(resp)
    uri = str(dc.get("verification_uri") or dc.get("verification_uri_complete") or "")
    ...
    interval = float(dc.get("interval", poll_seconds))
    deadline = time.monotonic() + max_wait
    grant = "urn:ietf:params:oauth:grant-type:device_code"
    token_body = (
        f"grant_type={grant}&device_code={device_code}"
        f"&client_id={CLIENT_ID}&client_secret={CLIENT_SECRET}"
    ).encode()
    while time.monotonic() < deadline:
        time.sleep(interval)
        status, _, resp = _post(
            f"{AUTH_BASE}/oauth/accesstoken",
            token_body,
            {
                "content-type": "application/x-www-form-urlencoded",
                "accept": "application/json",
        ...
        if status // 100 == 2 and data.get("access_token"):
            return data["access_token"]
        if data.get("error") not in ("authorization_pending", "slow_down", None):
            raise RuntimeError(f"token error: {data.get('error')}")
    raise TimeoutError("device-code authorisation timed out")
