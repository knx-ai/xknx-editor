...
class DynamicTreeBuilder:
    ...
    def _build(self, elem: object) -> DynamicNode | None:
        if isinstance(elem, ApplicationProgramDynamic):
            ...
            # ETS labels a block with its heading parameter's (ParamRefId) Text when it has one — e.g.
...
class DynamicUI:
    ...
    def _prune_inactive_channels(self) -> None:
        ...
        but ETS derives activeness from the stored group objects. Such sections are
    ...
    def active_parameter_driven_com_object_ref_ids(self) -> set[str]:
        ...
        the parameter-driven active set, matching what ETS materialises.
