"""Catalog browse tree: `parse_catalog_xml` yields the section hierarchy plus flat items
(only product / hardware2program ref-ids); the Registry does resolution and traversal.
"""
...
class CatalogSection:
    """Browse-tree node; `parent_id` is None at the top level."""
...
@dataclass(frozen=True, slots=True)
class CatalogItem:
    """Selectable entry: an orderable product with a hardware+program binding, resolved by id via the Registry."""
...
@dataclass(frozen=True, slots=True)
class CatalogDoc:
    """Parsed Catalog XML: section/item stores plus id-list edges."""
