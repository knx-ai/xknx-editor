...
# App-level schema leaves


@dataclass(frozen=True, slots=True)
class Parameter:
    """P-N parameter definition, no ParameterRef accessor."""
...
@dataclass(frozen=True, slots=True)
class ParameterType:
    """PT-{id}, optionally carrying an EN-N enumeration child."""
...
@dataclass(frozen=True, slots=True)
class AddressSpace:
    id: str  # typed value, e.g. "00C8" / "U045D2"
...
class ParameterCalc:
    id: int
    suffix: str | None = None  # optional, like "SceneNumber_1"
...
@dataclass(frozen=True, slots=True)
class ResourceSpec:
    """RS-{id} resource-style reference, app level."""

    id: str  # example: "01-00000"
...
@dataclass(frozen=True, slots=True)
class BitmapDef:
    """BD-{id} bitmap reference, app level."""
...
@dataclass(frozen=True, slots=True)
class Baggage:
    """BG-{id} baggage reference, manufacturer level (no app segment)."""
...
@dataclass(frozen=True, slots=True)
class Channel:
    id: str  # alphanumeric id, e.g. "1", "I1", "S10"
...
@dataclass(frozen=True, slots=True)
class ComObject:
    id: str  # inside ModuleDef this can be composite, e.g. "2-0"
...
@dataclass(frozen=True, slots=True)
class RepeatPath:
    """Chain across nested Repeat elements (X-N[_X-M...])."""
...
@dataclass(frozen=True, slots=True)
class SubModule:
    """M-N[_MI-N] inside a SubModuleDef scope."""
...
@dataclass(frozen=True, slots=True)
class SubModuleDef:
    """SM-N plus contents; sits under a ModuleDef or Module."""
...
@dataclass(frozen=True, slots=True)
class Module:
    """M-N[_MI-N] inside a ModuleDef scope."""
...
@dataclass(frozen=True, slots=True)
class Application:
    """Application program id A-{number:04X}-{version:02X}-{fingerprint:04X}[-{order}].

    Fields: number (4 hex), version (2 hex), fingerprint (4 hex),
    optional order suffix (e.g. "O000A"), and an optional app-level segment.
    """
...
@dataclass(frozen=True, slots=True)
class KnxId:
    """Manufacturer-level KNX id M-{manufacturer:04X}[_A-...|_BG-...].

    Holds the 4-hex manufacturer number plus one typed child: an Application, a
    Baggage, or None. Application nests further (ModuleDef/Module/SubModuleDef and
    the various parameter, com-object and allocator leaves), each node carrying a
    single typed child; from_string parses that chain and __str__ rebuilds it.
    """
    ...
    def from_string(s: str) -> KnxId:
        ...
        def parse_param_ref() -> ParamRef | None:
            ...
            if pos + 1 >= len(tokens) or not tokens[pos + 1].startswith("R-"):
                return None  # a P-N lacking R-N isn't a ParamRef
            p = hx(consume()[1])
        ...
        def parse_app_content() -> _AppContent | None:
            ...
            if p == "CH":
                return Channel(consume()[1])
            if p == "O":
                o_id = consume()[1]  # can be composite, e.g. "2-0"
                r = maybe("R")
                if r is not None:
                    return ComObjectRef(o_id, hx(r))
                return ComObject(o_id)
        ...
        if peek() == "A":
            app_str = consume()[1]  # forms like "9A1B-01-ABAF" or "7072-21-5CC3-O000A"
