...
class _StdlibBridgeHandler(logging.Handler):
    """Forwards stdlib log records from the ``xknxmono.*`` packages into a :class:`LogService`."""
...
class LogService:
    ...
    def _install_stdlib_bridge(self) -> None:
        """Route the packages' stdlib ``logging`` (``xknxmono.*`` - programming, download, myknx,
        ...
        """
        pkg_logger = logging.getLogger("xknxmono")
    ...
    def _ingest_stdlib(self, record: logging.LogRecord) -> None:
        ...
        # "xknxmono.download.procedure" -> "download"; anything else keeps its own short name.
        plugin = parts[1] if len(parts) > 1 and parts[0] == "xknxmono" else parts[0]
        payload = {
            k: str(v)
