"""The stdlib->LogService bridge that surfaces the ``xknxmono.*`` packages' logging (programming,
...
def test_bridge_forwards_package_logs_with_level_and_plugin():
    svc = LogService()
    logging.getLogger("xknxmono.download.procedure").debug(
...
def test_bridge_surfaces_extra_fields_as_payload():
    svc = LogService()
    logging.getLogger("xknxmono.project.core.myknx_cert").info(
...
def test_bridge_is_idempotent_across_instances():
    ...
    LogService()
    LogService()
    pkg = logging.getLogger("xknxmono")
