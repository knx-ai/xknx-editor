...
# manufacturer name. Basis is the online catalog manufacturer list (online_catalog_manufacturers.json);
# domains verified via manufacturer/KNX sources 2026-09. Used to build site-scoped documentation
# searches and to allow catalog_docs_fetch downloads. The tool always also returns a plain web search
# including the manufacturer name, so a manufacturer missing here degrades gracefully to the knx.org
# search rather than pointing nowhere.
#
# _domain_for returns the FIRST matching key, so order matters: put the more specific key first where
...
def _domain_for(manufacturer: str | None) -> str | None:
    if not manufacturer:
        return None
    name = manufacturer.lower()
    return next((d for kw, d in _MANUFACTURER_DOMAINS.items() if kw in name), None)
...
# (OpenKNX), and futurasmus-knxgroup (a KNX distributor whose descargas.* portal hosts the official
# manuals for makers who do not publish direct PDFs on their own site, e.g. Zennio). An allowlist
# keeps the server-side fetch a documentation tool, not an open SSRF proxy.
...
def register(mcp: FastMCP, ctx: McpContext) -> None:
    ...
    def catalog_docs(
        ...
        """Point to a device's technical documentation for ``query`` (returns links, NOT content).

        Give a topic like "dimming curve" or "channel A parameters". Supply ``product_ref_id`` to pull
        the manufacturer + order number from the catalog, or pass ``manufacturer`` directly. Returns
        ready-to-open links, best source first: the KNX device database (has the official certified
        DE/EN PDF only if the manufacturer uploaded it — otherwise it just links the manufacturer
        site), then the manufacturer site, OpenKNX for DIY hardware, and a plain web search.

        Match the exact order number AND series/generation (e.g. ``AKH-0400.02`` vs ``.03``): the
        manuals differ per series and a product page may show only the newest one. Feed a resulting
        PDF link to catalog_docs_fetch to read it. The tool does not fetch pages itself — see ``note``
        for how to use the links without blowing up context."""
        ...
        if product_ref_id:
            ...
            def _lookup() -> tuple[str | None, str | None]:
                ...
                return product.manufacturer_name, product.order_number

            resolved_mfr, order_number = ctx.run_locked(_lookup)
        ...
        return {
            ...
            "note": (
                "These are external pages and often LARGE PDF manuals; this tool returns links, not "
                "content. Do not load a whole manual into context — open the most specific link and "
                "extract only the part relevant to the query (e.g. the com-object table or one "
                "parameter). The references are ordered best-source-first and are FALLBACKS for each "
                "other: if one does not have the exact device/series or the answer, try the next "
                "(manufacturer site <-> KNX device database <-> web search). Only if none of them "
                "yields it, or you cannot open/fetch the documents yourself, ASK THE USER to open a "
                "link and paste the relevant section instead of guessing. To CONFIGURE a device, "
                "prefer project_list_parameters / project_list_com_objects: they already give "
                "ref_ids, current values and allowed values. Docs are mainly for human-readable "
                "explanations and wiring."
            ),
        }
