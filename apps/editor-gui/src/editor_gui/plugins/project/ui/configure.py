...
class ConfigurePanel:
    def __init__(
        ...
        # ETS-style "multi fill": when on, a parameter edit is applied to every device that runs
        # the same application program, not just the selected one.
        self._on_param_change_all = on_param_change_all
        self._apply_all = False
        # ETS-style "apply to all channels": mirror a parameter edit to the same parameter in every
        # other repeated channel of a device (PM 1, PM 2, …), for apps that expand channels.
        self._apply_all_channels = False
        # ETS-style multi-device edit: when >1 same-application device is selected in the tree, edits
    ...
    def render(self) -> None:
        ...
        if imgui.collapsing_header(
            ...
            # device.app.version is the KNX XML schema version (e.g. "20" for the ETS6 /20 schema),
    ...
    def _current_app_version(self, device: Device) -> int | None:
        ...
        real ETS application version; ``device.app.version`` is the KNX XML schema version instead."""
    ...
    def _render_update_action(
        ...
        """When a newer application version is available online, offer an ETS-style update that keeps
    ...
    def _render_multi_fill_toggle(self, device: Device) -> None:
        """ETS-style multi-fill toggle: apply parameter edits to all identical devices at once."""
