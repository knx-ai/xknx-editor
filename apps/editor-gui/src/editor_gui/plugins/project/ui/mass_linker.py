...
Instead of ETS's two positional lists, each object is a table row and its target is set explicitly
...
def first_free_values(
    ...
    Mirrors the ETS Bulk Linker's suggestion for object<->object links ("first available"
...
class MassLinkerPanel:
    ...
    def _assign_sequential(
        ...
        from xknxmono.project.core.addressing import parse_ga
        ...
        if imgui.begin_child("##ml_pick_tree", imgui.ImVec2(0.0, 340.0)):
            for device in self._get_devices():
                ...
                if imgui.tree_node_ex(header, imgui.TreeNodeFlags_.default_open):
                    # One click grabs every object of the device (the ETS "drop a device" speed-up).
