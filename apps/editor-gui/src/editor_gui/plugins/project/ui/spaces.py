...
def _function_type_presets() -> list[tuple[str, str]]:
    """(label, ETS FunctionType code) for the type picker. Codes verified against knx_master.xml
    (current, non-deprecated); role stays free text since ETS accepts group addresses without one."""
    return [
...
def _space_type_presets() -> list[tuple[str, str]]:
    """(label, ETS SpaceType code) for the type picker. Codes are the common, non-deprecated
