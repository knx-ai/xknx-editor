...
``xknx-project`` is catalog-free, so its :func:`~xknxmono.project.export_knxproj` writes only the
...
def _merge_masters(masters: list[bytes]) -> bytes | None:
    ...
    The blob with the most manufacturers is the base (it carries the full MasterData ETS needs);
    manufacturers present only in the others are appended to it.

    Genuine ``knx_master.xml`` carries a ``MasterData`` ``Signature`` over its exact bytes, and ETS
    rejects the whole import ("Invalid import data") when that signature no longer matches the
