...
A genuine ``knx_master.xml`` carries a ``MasterData`` ``Signature`` over its exact bytes. ETS
rejects the whole import ("Invalid import data") when that signature no longer matches, so the
