"""Keyring / KNX Secure panel: import a ``.knxkeys`` keyring (with password) and browse its
...
def _key_preview(value: Any) -> str:
    if not value:
        return "-"
...
class KeyringPanel:
    ...
    def render_import_prompt(self) -> None:
        ...
        This is the whole import UI — no big window. Call every frame."""
        self._poll_dialog()
        ...
        if (imgui.button(S.KEYRING_LOAD) or submitted) and self._pending_path:
            try:
                ...
                Exception
            ) as e:  # xknx raises InvalidSecureConfiguration on bad password
    ...
    def render_contents(self) -> None:
        ...
        imgui.text_disabled(
            S.KEYRING_HEADER.format(
                project=keyring.project_name or "?", by=keyring.created_by or "?"
            )
        )

        if keyring.backbone is not None and imgui.collapsing_header(
            S.KEYRING_BACKBONE, imgui.TreeNodeFlags_.default_open
        ):
            bb = keyring.backbone
            imgui.text_disabled(
                f"{bb.multicast_address}   key {_key_preview(bb.decrypted_key)}"
            )

        self._render_interfaces(keyring.interfaces)
        self._render_group_addresses(keyring.group_addresses)
        self._render_devices(keyring.devices)

    def _render_interfaces(self, interfaces: list[Any]) -> None:
        ...
        if imgui.begin_table("##kr_ifaces", 4, flags):
            ...
            for iface in interfaces:
                ...
                imgui.text(str(getattr(iface, "type", "") or ""))
    ...
    def _render_group_addresses(self, group_addresses: list[Any]) -> None:
        if not group_addresses or not imgui.collapsing_header(
            S.KEYRING_GROUP_ADDRESSES.format(count=len(group_addresses))
        ):
            return
        ...
        if imgui.begin_table("##kr_gas", 3, flags):
            ...
            for ga in group_addresses:
                ...
                imgui.text(addr)
                imgui.table_set_column_index(1)
                imgui.text_disabled(_key_preview(ga.decrypted_key))
                imgui.table_set_column_index(2)
                if addr in secure:
                    imgui.text("✓")
            imgui.end_table()

    def _render_devices(self, devices: list[Any]) -> None:
        ...
        if imgui.begin_table("##kr_devices", 2, flags):
            ...
            for dev in devices:
                ...
                imgui.table_set_column_index(1)
                imgui.text_disabled(_key_preview(dev.decrypted_tool_key))
            imgui.end_table()
