...
Before building any ETS-parity feature, assume it may already exist and grep first. Current state:
...
- **Localization**: gettext catalogs per plugin domain under `<plugin>/locales/<lang>/LC_MESSAGES/<domain>.mo` (de + nl); UI language auto-detected from OS locale, overridable via the Language menu (persisted in `app.locale`), runtime-switchable. Device parameter/tab labels are localized from the `.knxprod`'s own translations (`xknxmono.product.translate.apply_translations`, language threaded through `catalog.get_application`).
...
- **KNX master data**: a bundled `assets/knx_master.xml` (the global mask-version load/unload procedures + manufacturers + DPTs ETS ships into every `.knxprod`) is loaded once at startup by `master_data.py::load_master` (prefers a fetched cache in `config/knx_master.xml` over the bundle), injected into `ConnectionService.master`, and threaded to `download`/`preflight` (required for an UNLOAD scope and default/merged procedure styles). Its version + date show in the status bar footer (`STATUS_MASTER_DATA`). Conditional-GET refresh from update.knx.org is not wired yet.
...
- **MCP** (`packages/mcp`, `xknx-mcp`): FastMCP 3.4.7 server over Streamable HTTP exposing catalog + project tools; launched/stopped from the app (host/port persisted in `config/mcp.json`, subprocess `uv run xknx-mcp`, shares this app's catalog DB); status shown in the status bar.
